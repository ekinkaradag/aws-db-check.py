    ...
    while True:
        ...
        for i, ip in enumerate(ipAddrs):
            ...
            if "Cassandra" in names[i]:
                ...
                p = subprocess.Popen(["sqlplus", oracle11g_user + "/" + oracle11g_pass + "@" + ip + ":" +oracle11g_port + "/" + oracle11g_servicename , "as", oracle11g_role],stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
                p.communicate("exit")
                if p.returncode != 0:
                    stdscr.addstr(2+i, 20, names[i][-9:], curses.color_pair(2)) #Print the machines name in RED
                else:
                    stdscr.addstr(2+i, 20, names[i][-9:], curses.color_pair(3)) #Print the machines name in GREEN
                    stdscr.addstr(2+i, 50, str(oracle11g_port), curses.color_pair(1)) #Print checked port
                    stdscr.addstr(2+i, 60, str(oracle11g_user), curses.color_pair(1)) #Print username
                    stdscr.addstr(2+i, 75, str(oracle11g_pass), curses.color_pair(1)) #Print password
                    stdscr.addstr(2+i, 90, str(oracle11g_role), curses.color_pair(1)) #Print role
                    stdscr.addstr(2+i, 100, str(oracle11g_servicename), curses.color_pair(1)) #Print service name
